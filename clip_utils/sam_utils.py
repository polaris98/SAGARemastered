...
def create(clip_model, image_list, data_list, save_folder):
    ...
    for i, img in tqdm(enumerate(image_list), desc="Embedding images", leave=False):
        timer += 1
        try:
            img_embed, seg_map = _embed_clip_sam_tiles(clip_model, img.unsqueeze(0), sam_encoder)
        except:
...
def _embed_clip_sam_tiles(clip_model, image, sam_encoder):
...
if __name__ == '__main__':
    ...
    save_folder = os.path.join(args.image_root, 'language_features')
    os.makedirs(save_folder, exist_ok=True)

    # import freelance_utils, importlib
    # importlib.reload(freelance_utils.clip_utils)
    from clip_utils import load_clip

    clip_model = load_clip()
    clip_model.eval()

    create(clip_model, imgs, data_list, save_folder)