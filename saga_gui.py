...
class GaussianSplattingGUI:
    ...
    def pca(self, X, n_components=3):
        ...
        eigenvectors = eigenvectors[:, idx].real  # Take the real part of eigenvectors!
    ...
    def fetch_data(self, view_camera):
        ...
        if len(self.new_click_xy) > 0:
            ...
            if self.segment3d_flag:
                ...
                print(self.score_pts_binary.shape)
                ...
                # clean_mask = self.engine['scene'].post_clean()
                # self.engine['scene'].segment(clean_mask)
                # self.engine['feature'].segment(clean_mask)
                self.engine['scene'].save_ply(os.path.join(opt.MODEL_PATH, f'point_cloud/iteration_60000/scene_point_cloud.ply'))
