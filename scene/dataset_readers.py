...
def readColmapCameras(cam_extrinsics, cam_intrinsics, images_folder, features_folder = None, masks_folder = None, mask_scale_folder = None, sample_rate = 1.0, allow_principle_point_shift = False,iteration=None):
    cam_infos = []
    for idx, key in enumerate(cam_extrinsics):
        ...
        if(mask_scale_folder is not None):
            if(iteration is None):
                mask_scales = torch.load(os.path.join(mask_scale_folder, image_name.split('.')[0] + ".pt"))
            else:
                mask_scales = torch.load(os.path.join(mask_scale_folder+"/iteration_"+str(iteration), image_name.split('.')[0] + ".pt")) 
...
def readColmapSceneInfo(path, images, eval, llffhold=8, need_features=False, need_masks=False, sample_rate = 1.0, allow_principle_point_shift = False, replica=False,iteration=None):
    try:
