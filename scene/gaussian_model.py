...
class GaussianModel:
    ...
    def set_all_colors_to_red(self):
        if self._xyz.nelement() == 0:  # Check if the model is initialized
            print("GaussianModel is not initialized yet. Call create_from_pcd first.")
            return

        red_color = torch.tensor([1.0, 0.0, 0.0]).float().cuda()
        red_sh = RGB2SH(red_color.unsqueeze(0))
      
        # Important: Ensure the shape is correct before assigning
        num_points = self._xyz.shape[0]
        num_sh_coeffs = self._features_dc.shape[2] # or (self.max_sh_degree + 1)**2
        
        new_features_dc = red_sh.repeat(num_points, 1, num_sh_coeffs).transpose(1, 2).contiguous()

        #Check if the tensor is on the correct device
        if self._features_dc.device != new_features_dc.device:
            new_features_dc = new_features_dc.to(self._features_dc.device)
        
        # Replace the existing _features_dc with the new red features
        self._features_dc = nn.Parameter(new_features_dc.requires_grad_(self._features_dc.requires_grad))
    ...
    def save_ply(self, path):
        ...
        # #Create red color and convert to SH
        # red_color = torch.tensor([1.0, 0.0, 0.0]).float().to(self._features_dc.device)
        # red_sh = RGB2SH(red_color.unsqueeze(0)).cpu().numpy()

        # # Create an array of correct shape with the red color
        # f_dc = np.repeat(red_sh, num_points, axis=0)
    ...
    def segment(self, mask=None):
        ...
        print("mask_segement", mask.shape)
    ...
    def post_clean(self):
        xyz = self.get_xyz
        nearest_k_idx = pytorch3d.ops.ball_query(
                    xyz.unsqueeze(0),
                    xyz.unsqueeze(0),
                    K=4,
                    radius=0.075,
                ).idx.squeeze()
        valid_idx = torch.ones((xyz.shape[0],), dtype=torch.bool, device="cuda")
        mask = nearest_k_idx[:,-1] != -1
        valid_mask = valid_idx*mask
        return valid_mask   
