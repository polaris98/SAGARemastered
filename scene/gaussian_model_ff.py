...
class FeatureGaussianModel:
    ...
    def get_smoothed_point_features(self, K = 16, dropout = 0.5):
        ...
        with torch.no_grad():
            if self.feature_smooth_map is None or self.feature_smooth_map["K"] != K:
                ...
                distance_map = nearest_k_map.dists.squeeze()
                nearest_k_idx = nearest_k_map.idx.squeeze()
                self.feature_smooth_map = {"K": K, "m": nearest_k_idx, "d":distance_map}
    ...
    def get_smoothed_point_features_m(self, K = 16, dropout = 0.5):
        Z=0.1
        if K <= 1:
            return self._point_features
        ...
        with torch.no_grad():
            if self.feature_smooth_map is None or self.feature_smooth_map["K"] != K:
                ...
                #distance_map = nearest_k_map.dists.squeeze()
                nearest_k_idx = nearest_k_map.idx.squeeze()*(nearest_k_map.dists.squeeze()<Z)
                self.feature_smooth_map = {"K": K, "m": nearest_k_idx}
    ...
    def get_smoothed_point_features_m2(self, K = 16, dropout = 0.5):
        Z=0.01
        if K <= 1:
            return self._point_features
        ...
        with torch.no_grad():
            if self.feature_smooth_map is None or self.feature_smooth_map["K"] != K:
                ...
                select_point = torch.arange(0, 16)
                select_idx = nearest_k_idx[:,select_point]
                valid_mask = select_idx != -1  # Shape: [5000, 16]

                # Gather features using valid indices
                safe_select_idx = torch.where(select_idx == -1, torch.tensor(0, device=select_idx.device), select_idx)
                self.feature_smooth_map = {"K": K, "m": nearest_k_idx, "valid_mask": valid_mask, "safe_select_idx":safe_select_idx}

        normed_features = torch.nn.functional.normalize(self._point_features, dim = -1, p = 2)
        if dropout > 0 and dropout < 1:
          
            gathered_features = normed_features[self.feature_smooth_map["safe_select_idx"]]  # Shape: [5000, 16, 64]

            # Mask out invalid features (corresponding to -1 in select_idx)
            masked_features = gathered_features * self.feature_smooth_map["valid_mask"].unsqueeze(-1)  # Shape: [5000, 16, 64]

            # Count valid indices per row
            valid_counts = self.feature_smooth_map.sum(dim=1, keepdim=True).clamp(min=1)  # Shape: [5000, 1]

            # Compute the mean of valid features
            ret = masked_features.sum(dim=1) / valid_counts  # Shape: [5000, 64]
