...
def compute_nbrs( pc : FeatureGaussianModel,K):
    with torch.no_grad():
        if pc.feature_smooth_map is None or pc.feature_smooth_map["K"] != K:
            xyz = pc.get_xyz
            ...
            pc.feature_smooth_map = {"K":K, "m":nearest_k_idx}

from sklearn.neighbors import NearestNeighbors
            
def compute_knn_distance_loss(distances, indices, gaussian_features, k, weight, sigma):   
    # Compute feature differences and distance weights
    feature_diff = gaussian_features.unsqueeze(1) - gaussian_features[indices]  # NxKxD
    feature_similarity = torch.sum(feature_diff**2, dim=-1)  # NxK

    distance_weights = torch.exp(-distances / sigma)

    # Compute KNN distance loss
    knn_loss = (distance_weights * feature_similarity).mean()
   
    return weight * knn_loss
...
def training(dataset, opt, pipe, iteration, saving_iterations, checkpoint_iterations, debug_from):
    ...
    for i in range(len(iter_list)):
        print("self distill iteration", iter_list[i])

        if(i!=0):
            prev_gaussians = GaussianModel(dataset.sh_degree)
            prev_feature_gaussians = FeatureGaussianModel(dataset.feature_dim) 
            scene = Scene(dataset, prev_gaussians, prev_feature_gaussians, load_iteration=iter_list[i-1], shuffle=False, target='contrastive_feature', mode='train', sample_rate=sample_rate)
           
        gaussians = GaussianModel(dataset.sh_degree)
        feature_gaussians = FeatureGaussianModel(dataset.feature_dim) 
        scene = Scene(dataset, gaussians, feature_gaussians, load_iteration=iter_list[i], shuffle=False, target='contrastive_feature', mode='train', sample_rate=sample_rate)
        ...
        # Find KNN
        distances = nearest_k_map.dists.squeeze()
        indices = nearest_k_map.idx.squeeze()
        #############################################################

        ###################INIATIALIZING SELF DISTILLATION##################
        if(i!=0):
            print("Initializing self distillation...")
            prev_xyz = prev_gaussians.get_xyz.detach()
            
            nearest_k_map_teacher = pytorch3d.ops.knn_points(
            xyz.unsqueeze(0),
            prev_xyz.unsqueeze(0),
            K=1  
            )
            self_teacher_indices = nearest_k_map_teacher.idx.squeeze()  
            print("Self distillation initialized.")
            ##################################################################
        ...
        for it in range(first_iter, opt.iterations + 1):
            iter_start.record()
            ...
            ################################DISTANCE ADN SELF DISTILLATION LOSS##########################  
            
            distance_loss = compute_knn_distance_loss(distances, indices, feature_gaussians.get_point_features, k, 100, 1.0)
            loss += distance_loss

            if i!= 0 and it <int(15*(i/len(iter_list))): 
                print("computing self distillation loss...")
                f_teacher_matched = prev_feature_gaussians.get_point_features[self_teacher_indices, :]
                distill_weight = 1.0
                distill_loss = torch.nn.functional.mse_loss(feature_gaussians.get_point_features, f_teacher_matched).cuda()
        ...
        print("end of iteration", iter_list[i])
       
        scene.save_feature(iter_list[i], target = 'contrastive_feature', smooth_weights = torch.softmax(smooth_weights, dim = -1) if smooth_weights is not None else None, smooth_type = 'traditional', smooth_K = opt.smooth_K)
        torch.save(scale_gate.state_dict(), os.path.join(scene.model_path, "point_cloud/iteration_{}/".format(iter_list[i]) + "scale_gate.pt"))
